    ...
    public static class BridgeHttpServer
    {
        ...
        private static void HandlePing(HttpListenerContext context)
        {
            if (!BridgeSession.IsInitialized)
            {
                ...
                var fallback = "{\"connected\":false,\"projectPath\":null,\"unityVersion\":null,\"bridgeVersion\":\"1.2.2\",\"wireContract\":"
        ...
        private static GateDispatchResult DispatchWithGate(string toolName, string body, string gateMode, string[] pathsHint)
        {
            ...
            // No try/catch around DispatchWithGateCore: the capture is purely
            // index-based (StartCapture/StopCapture hold no shared window
            // state — the baseline is the caller-held `captureStart` int), so
            // an exception leaves nothing open and the fault envelope is
