...
//
// Disable state is persisted in `.unity-agent/settings.json` via `BridgeProjectSettings` and
...
namespace UnityAgentBridge
{
    public static class BridgeToolTogglePolicy
    {
        ...
        {
            var msg = $"Tool '{toolName}' is disabled in the Unity Agent Bridge runtime. " +
                      "Re-enable it in the Tools tab (or remove it from `.unity-agent/settings.json` `disabledTools`) and retry.";
            return $"{{\"error\":{{\"code\":\"{DisabledErrorCode}\",\"message\":\"{EscapeStringContent(msg)}\",\"tool\":\"{EscapeStringContent(toolName)}\",\"hint\":\"Enable the tool in the Unity Agent Bridge window Tools tab.\"}}}}";
        }
