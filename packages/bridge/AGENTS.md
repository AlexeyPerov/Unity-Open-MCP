...
- Shipped domain tools (Nav, Input, ProBuilder, Particles, Animation) live under `Editor/TypedTools/Extensions/<Domain>/`, **not** in `packages/extensions/`. See [Embedded domain model](../../docs/contributing/extensions.md#embedded-domain-model).
...
- `Editor/Gate/GatePolicy.cs` (`GatePolicy.Execute`) is canonical for gate precedence: request `gate` → project default (`BridgeGateDefaultPolicy`) → tool attribute default. If externally visible precedence changes, update the owning public architecture/API documentation in the same task.
...
- **Hand-rolled JSON must use the shared `BridgeJson` appenders.** New code that builds a JSON value token into a `StringBuilder` MUST use `BridgeJson.AppendJsonString` / `AppendJsonBool` / `AppendJsonNumber` (or the `AppendJson*Field` variants for `"key":value` pairs) instead of re-implementing the escape switch or calling `sb.Append(bool)` / `sb.Append("...")` inline. Two recurring bug classes this prevents: (1) **split strings** — closing a JSON string literal in one `Append` and reopening it in another leaves the second half as a bare token and the whole body is rejected as `bridge_response_unparsable` (the M30 Plan 1 profiler `note` bug); (2) **bool casing** — `sb.Append(bool)` emits C# `True`/`False`, not JSON `true`/`false` (the historical `asmdef_list` autoReferenced bug). For escaped string *content* (no surrounding quotes), use `BridgeJson.EscapeStringContent`. The appenders are unit-tested in `Tests/Editor/Bridge/BridgeJsonTests.cs`. Extension sub-assemblies (`Editor/TypedTools/Extensions/*/*Json.cs`) are separate assemblies and keep their own per-extension escape helper — `BridgeJson` is `internal` to the root assembly; do not make it public just to reach an extension. **The one sanctioned second escape helper is `OutputSerializer.EscapeJsonString`** (the reflection serializer): it has ~80 call sites across the reflection tools and intentionally diverges from `BridgeJson` (`\b`/`\f` short escapes vs `\uXXXX`). It is documented in place; do not introduce a third escape helper.
...
- **Lock retention on domain reload.** `BridgeHttpServer.ForceStopListener(releaseLock: false)` is called from `beforeAssemblyReload` (and during port-in-use recovery); the lock is only deleted on graceful quit (`EditorApplication.quitting` → `Stop(releaseLock: true)`). `Stop()` delegates to `ForceStopListener` and no longer skips listener teardown when `_running` is false but `_listener` / the worker thread still exist. This is deliberate: when the bridge assembly itself fails to compile, `[InitializeOnLoad]` never re-runs and the heartbeat stops advancing, leaving a lock whose PID is still alive (Unity is running, just stuck) but whose `heartbeatAt` is frozen. That stale-heartbeat + live-PID signature is the ONLY out-of-band signal the MCP server has to detect a dead bridge and fail fast instead of hanging on `/ping` (see `classifyInstance` in `mcp-server/src/instance-discovery.ts` and the `bridge_compile_failed` error in `mcp-server/src/live-client.ts`). Do not re-introduce a lock release on the reload path. **Port-in-use recovery** (M26 Plan 4) retries bind in-process only — never terminate foreign PIDs.
- **Skip the listener in worker / batch processes.** Unity loads this assembly in child processes it spawns for asset import / background work (`AssetImportWorker*`, launched `-batchMode -parentPid`). The `[InitializeOnLoad]` static ctor (`BridgeHttpServer`) gates `Start()` (and the reload/quit hook subscription) behind `IsWorkerOrBatchProcess` (`Application.isBatchMode`); a worker logs one `Skipping listener in batch/worker process (...)` line and returns. Without this guard the worker bound the project's deterministic port and the main Editor could not bind — the recurring "Address already in use" under heavy automation, surfacing as a confusing `dead_bridge` while `/ping` still succeeded against the worker's listener. `Application.isBatchMode` is used (not `-parentPid`) because the bridge floor is Unity 2022.3 and `-parentPid` is Unity-6-only; the deliberate `BridgeBatchEntry` headless path is also `-batchmode` but never relied on this listener (`-executeMethod`-driven, self-exits), so gating it out is harmless. When `Start()` fails terminally but a lock is already held in-process, the heartbeat is restarted so the lock advances instead of freezing at `reloading` (defense in depth; never mint a new lock on the failure path — the contract is to never advertise a port nothing is listening on).
