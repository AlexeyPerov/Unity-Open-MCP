// M18 Plan 3 — Animation embedded domain tools EditMode tests.
//
// Ported from packages/extensions/animation/Tests/Editor (former standalone
// extension pack). Gated by UNITY_OPEN_MCP_EXT_ANIMATION via the owning test
// asmdef's defineConstraints, so the suite only compiles + runs when the
// built-in animation module is present (it is on every modern Unity install).
