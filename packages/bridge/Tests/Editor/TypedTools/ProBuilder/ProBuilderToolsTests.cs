// M18 Plan 3 — ProBuilder embedded domain tools EditMode tests.
//
// Ported from packages/extensions/probuilder/Tests/Editor (former standalone
// extension pack). Gated by UNITY_OPEN_MCP_EXT_PROBUILDER via the owning test
// asmdef's defineConstraints, so the suite only compiles + runs when the
// ProBuilder package is present — matching the compile-gate on the tool code
// under test.
