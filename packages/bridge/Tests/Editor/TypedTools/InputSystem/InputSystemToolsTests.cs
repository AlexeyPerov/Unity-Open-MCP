// M18 Plan 3 — Input System embedded domain tools EditMode tests.
//
// Ported from packages/extensions/inputsystem/Tests/Editor (former standalone
// extension pack). Gated by UNITY_OPEN_MCP_EXT_INPUTSYSTEM via the owning test
// asmdef's defineConstraints, so the suite only compiles + runs when the
// Input System package is present — matching the compile-gate on the tool
// code under test.
