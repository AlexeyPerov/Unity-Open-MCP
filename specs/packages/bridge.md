...
## Hard dependency on verify package (M3+)

Bridge `package.json` declares a UPM dependency on `com.alexeyperov.unity-agent-verify`. There is no soft-degraded gate path — if the verify package is absent, the project fails at compile time because `VerifyGateAdapter` directly references types from `UnityAgentVerify` (no reflection, no optional integration). This is intentional per Q8 A: architecture separates packages but M3+ gate requires verify.

Demo `manifest.json` wires both packages via `file:` references.
