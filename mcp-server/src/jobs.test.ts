...
test("always-visible local jobs work without bridge, reject arbitrary tools, and isolate routed identities", async t => {
  ...
  assert.equal(body(await router.route(name, { action: "start", tool_or_command: "unity_open_mcp_execute_csharp", idempotency_key: "x" })).error.code, "job_operation_unsupported");
  ...
  for (const identity of [b, otherPort]) {
    ...
    assert.equal(body(await router.routeJobs({ action: "status", job_id: job.job_id }, identity)).error.code, "job_not_found");
  }
