...
export const terrainCreate = makeTool(
    ...
    "dependency); the terrain group is hidden until manage_tools activates it.",
  {
    required: ["paths_hint"],
        properties: {
          ...
          paths_hint: { ...PATHS_HINT_TYPE, description: "Mutation scope — the new terrain's scene path. When asset_path is " + "provided, include BOTH the scene path and the .asset path so the gate " + "validates both. No whole-project fallback." },
          gate: { ...GATE_PROP },
        },
  },
);
