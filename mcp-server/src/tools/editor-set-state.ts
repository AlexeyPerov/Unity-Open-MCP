...
export const editorSetState: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      ignore_scene_dirty: {
        ...
        default: false,
        description:
          "Opt out of the active-scene dirty guard. When false (default), the " +
          "call is refused with code `scene_dirty` if any loaded scene has " +
          "unsaved changes — entering play mode would otherwise trigger " +
          "Unity's native save modal. Set true to proceed and accept the risk.",
      },
    },
