...
export const buildSetTarget: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      paths_hint: {
        ...
        items: { type: "string" },
        description:
          "Mutation scope — pass [\"ProjectSettings\"] so the gate covers the rewritten " +
          "settings assets. There is no whole-project fallback.",
      },
      gate: {
        enum: ["enforce", "warn", "off"],
        default: "enforce",
        description: "Gate mode. Default 'enforce' — fails the call if the switch surfaces new errors.",
      },
      ignore_scene_dirty: {
        type: "boolean",
        default: false,
        description:
          "Bypass the active-scene dirty guard. By default a disruptive op (recompile / scene " +
          "switch) is refused with scene_dirty when any loaded scene has unsaved changes, so " +
          "Unity's native save modal never interrupts the flow. Set true to proceed and accept " +
          "the risk of a native save prompt.",
      },
    },
