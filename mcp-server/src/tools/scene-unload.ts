...
export const sceneUnload = makeTool(
    ...
    "Prefer this over raw execute_csharp SceneManager.UnloadSceneAsync.",
  {
    required: ["paths_hint"],
        properties: {
          ...
          paths_hint: { ...PATHS_HINT_TYPE, description: "Mutation scope — the unloaded scene's asset path (or its name when it is not yet " + "asset-backed)." },
          gate: { ...GATE_PROP },
        },
  },
);
