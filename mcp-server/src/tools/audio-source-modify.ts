...
export const audioSourceModify = makeTool(
    ...
    "when mixer_group_path is set).",
  {
    required: ["paths_hint"],
        properties: {
          ...
          paths_hint: { ...PATHS_HINT_TYPE, description: "Mutation scope — the host scene path (and the mixer asset path when " + "mixer_group_path is set). No whole-project fallback." },
          gate: { ...GATE_PROP },
        },
  },
);
