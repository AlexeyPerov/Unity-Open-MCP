...
// fallout.
export const settingsSetLighting = makeTool(
    ...
    "applied keys + any per-key warnings.",
  {
    required: ["fields", "paths_hint"],
        properties: {
          ...
          paths_hint: { ...PATHS_HINT_TYPE, description: "Mutation scope — pass the active scene path (the scene gets dirtied on each write) " + "and/or the render/lighting settings asset. There is no whole-project fallback." },
          gate: { ...GATE_PROP },
        },
  },
);
