...
export const assetsCreateFolder = makeTool(
    ...
    "folder path (e.g. [\"Assets/NewFolder\", \"Assets/NewFolder/Sub\"]).",
  {
    required: ["folders", "paths_hint"],
        properties: {
          ...
          paths_hint: { ...PATHS_HINT_TYPE, description: "Created folder paths (the gate's validation scope). There is no whole-project fallback." },
          gate: { ...GATE_PROP, description: "Gate mode. Default 'enforce' — fails the call if the create surfaces new errors." },
        },
  },
);
