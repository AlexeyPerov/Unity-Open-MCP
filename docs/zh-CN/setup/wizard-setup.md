...
向导顶部显示可点击的进度条。当某段的检查已通过时，该段变为**绿色**，便于一眼看出哪些步骤仍需处理。你可以点击任意段落跳转到该步骤；Back/Next 仍按顺序移动。无论以何种方式离开某一步骤 — Next、Back 或点击进度条跳转 — 该步骤中输入的值都会被应用，因此不会因为跳转而丢失已填内容。推荐路径在任何步骤都不需要展开 **Advanced (optional)**。
...
| 预设 | 最适合 | 预填内容 |
|---|---|---|
| **Regular user (npm)** *(推荐)* | 想使用已发布 npm 包、不检出 monorepo 的开发者 | `npx -y unity-open-mcp@1.0.0`；bridge + verify 来自已发布源；领域依赖关闭；技能开启 |
