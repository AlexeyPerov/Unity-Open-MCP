...
Каждый фрагмент на этой странице содержит **ваш** абсолютный путь к проекту,
поэтому такой файл остаётся на вашей машине. Чтобы закоммитить одну конфигурацию
на всю команду, см. [Переносимую конфигурацию MCP](portable-config.md).
...
`<project>` — это папка, на которой открыт AI-клиент, и она не всегда совпадает
с Unity-проектом. Если Unity-проект лежит во вложенной папке большего
репозитория (`<repo>/Client`), положите конфиг в корень репозитория, а
`UNITY_PROJECT_PATH` оставьте указывающим на папку Unity:

```
my-game/                    <- здесь открыт AI-клиент, здесь же конфиг
  .cursor/mcp.json          <- UNITY_PROJECT_PATH = /abs/my-game/Client
  Client/                   <- Unity-проект (Assets/, Packages/)
  Server/
```

Панель **Configure AI client** в окне моста просматривает сам Unity-проект и до
четырёх родительских папок, поэтому такая раскладка показывает
**Configured: yes** и называет найденный файл в поле **Found in**.

Конфигурация в корне репозитория вообще не обязана содержать абсолютный путь —
форма для коммита описана в [Переносимой конфигурации MCP](portable-config.md).
...
Сервер берёт первое из заданного и приводит результат к абсолютному пути:

| Приоритет | Вход | Результат |
|---|---|---|
| 1 | `--project <path>` (только подкоманды CLI) | абсолютный либо относительно рабочего каталога |
...
`UNITY_PROJECT_PATH` важнее `--project-from-cwd`, поэтому переопределение через
окружение всё ещё перенаправляет конфигурацию, использующую флаги. Итоговый путь
и то, какой вход его дал, пишутся в stderr при старте. Именно варианты 3 и 4
позволяют закоммитить конфигурацию — [Переносимая конфигурация
MCP](portable-config.md).

## Опционально

| Переменная | Обязательно | Назначение |
|---|---|---|
| `UNITY_PROJECT_PATH` | да, если не передан `--project-from-cwd` | Корень проекта Unity: абсолютный либо относительно рабочего каталога. |
| `UNITY_OPEN_MCP_BRIDGE_PORT` | нет | Зафиксировать порт моста вместо поиска по пути. Зависит от машины — не коммитьте. |
| `UNITY_PATH` | нет | Явный исполняемый файл Unity для пакетного отката. |
| `UNITY_OPEN_MCP_ALLOW_VERSION_MISMATCH` | нет | Значение `1` явно разрешает batch fallback или обработчику стартовых диалогов открыть проект в версии Unity, отличной от `ProjectVersion.txt`; по умолчанию несовпадение блокируется. |
...
**Общая конфигурация в коммите:** [Переносимая конфигурация
MCP](portable-config.md) заменяет абсолютный путь на `${workspaceFolder}` или
`--project-from-cwd`, чтобы один файл работал у всех.
