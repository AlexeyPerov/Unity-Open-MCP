...
The `unity-open-mcp setup` CLI and the **Hub AI Setup wizard** write skills into
your project at the per-client path each MCP client expects. The CLI copies the
template bytes bundled in the npm package; the wizard copies them from its
toolkit source. Destination paths are defined once in
[`skills/client-paths.json`](../skills/client-paths.json) — the single source of
truth consumed by setup, the Hub wizard, and the runtime skill generator. You
do not edit this file by hand.

The npm build copies the canonical `skills/unity-open-mcp/SKILL.md` bytes into
`dist/skill/SKILL.md`; the published package therefore needs no GitHub download
to install the playbook. A package test checks that the two files match.
