...
{
  "dependencies": {
    "com.alexeyperov.unity-open-mcp-bridge": "https://github.com/AlexeyPerov/unity-open-mcp.git?path=packages/bridge#bridge-v0.4.1",
    "com.alexeyperov.unity-open-mcp-verify": "https://github.com/AlexeyPerov/unity-open-mcp.git?path=packages/verify#verify-v0.4.1"
  }
}
```
...
- `npx -y unity-open-mcp@0.4.1` downloads and launches that exact MCP server
...
```bash
npx -y unity-open-mcp@0.4.1 wait-for-ready --project /path/to/MyGame
npx -y unity-open-mcp@0.4.1 run-tool unity_open_mcp_capabilities --project /path/to/MyGame --json
```
