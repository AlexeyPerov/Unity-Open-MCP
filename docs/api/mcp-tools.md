...
> **Install / connect.** The MCP server ships on npm as [`unity-open-mcp`](https://www.npmjs.com/package/unity-open-mcp). Most users never install it manually — the AI client spawns it via `npx -y unity-open-mcp@0.5.0` (no repo clone required). See [Manual setup](../setup/manual-setup.md) for the full client-config snippets and environment variables, and the [Maintainer publish flow](../setup/development-setup.md#maintainer-publish-flow) for how the package is published and updated.
