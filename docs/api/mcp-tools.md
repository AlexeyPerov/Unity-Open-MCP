...
> **Install / connect.** The MCP server ships on npm as [`unity-open-mcp`](https://www.npmjs.com/package/unity-open-mcp). Most users never install it manually — the AI client spawns it via `npx -y unity-open-mcp@0.5.0` (no repo clone required). See [Manual setup](../manual-setup.md) for the full client-config snippets and environment variables, and the [Maintainer publish flow](../development-setup.md#maintainer-publish-flow) for how the package is published and updated.
...
- **Most tools, including every `batchCapable: true` tool except `compile_check`, route `live`.** Batch is a *fallback* used only when `live.isLiveAvailable()` is false (`_route.fallbackReason: "live_unavailable"`).
...
- Keep some tools route-pinned:
  ...
  - `unity_open_mcp_read_compile_errors` always uses offline. Reads `Editor.log` for CSxxxx compiler errors + package/assembly red flags. When the response carries `staleLogSuspected: true`, the cited source files were edited more recently than the log — the error block may reference on-disk code you have already fixed (Unity's incremental compiler no-op'd the recompile). Force a recompile via `reimport_package` (local package) or `compile_check` before trusting the errors. The response also carries `staleLogHint` + `staleLogNewerFiles[]` (the offending files).
...
| `editor_instance_locked` | `compile_check` while a live Editor holds the project lock | Response carries a structured `agentNextSteps[]`. Close the live Editor for a headless check, or verify compile state without closing via `read_compile_errors`, or `reimport_package` (with `dllMtimeBefore`/`After`) to confirm a specific local package recompiled. |
