...
const NPM_RESOLVED_LABEL: &str = "npm: unity-open-mcp@1.2.2";
...
///
/// - `Npx` — `command: npx, args: ["-y", "unity-open-mcp@1.2.2"]`.
...
fn build_entry_json(params: &McpConfigParams, resolved_index: &str) -> Value {
    ...
    match params.client {
        ...
        McpClientId::OpencodeGlobal | McpClientId::OpencodeProject => {
            ...
            // resolved command so `npx -y unity-open-mcp@1.2.2` becomes
            // `["npx", "-y", "unity-open-mcp@1.2.2"]` and
...
/// keyed off the launch mode — `npx -y unity-open-mcp@1.2.2`
...
"#,
    ...
    fn plan_npx_mode_emits_npx_command_without_toolkit_root() {
        ...
        assert_eq!(args[0], "-y");
        assert_eq!(args[1], "unity-open-mcp@1.2.2");
...
"#,
    ...
    fn claude_mcp_add_command_npx_mode_omits_node_prefix() {
        ...
        assert!(cmd.contains("-- npx -y unity-open-mcp@1.2.2"));
        assert!(!cmd.contains("-- node "));
    }
