...
use crate::config::constants::{
    NPM_PACKAGE, PORT_ENV_VAR, PROJECT_PATH_ENV_VAR, UNITY_PATH_ENV_VAR,
};
...
const NPM_RESOLVED_LABEL: &str = "npm: unity-open-mcp@0.8.3";
...
///
/// - `Npx` — `command: npx, args: ["-y", "unity-open-mcp@0.8.3"]`.
...
fn launch_command_parts(mode: McpLaunchMode, resolved_index: &str) -> (String, Vec<String>) {
    match mode {
        McpLaunchMode::Npx => (
            "npx".to_string(),
            vec!["-y".to_string(), NPM_PACKAGE.to_string()],
        ),
...
fn build_entry_json(params: &McpConfigParams, resolved_index: &str) -> Value {
    ...
    match params.client {
        ...
        McpClientId::OpencodeGlobal | McpClientId::OpencodeProject => {
            ...
            // resolved command so `npx -y unity-open-mcp@0.8.3` becomes
            // `["npx", "-y", "unity-open-mcp@0.8.3"]` and
...
/// keyed off the launch mode — `npx -y unity-open-mcp@0.8.3`
...
pub struct SkillCopyPlan {
    ...
    pub toolkit_root: String,
    pub source_path: Option<String>,
    /// Full text of the template skill when the source file is readable —
    /// the wizard shows this in a Preview disclosure before writing.
...
fn plan_skill_copy_at(params: &SkillCopyParams) -> Result<SkillCopyPlan, SkillCopyError> {
    ...
    let source_preview = source_path
        .as_ref()
...
"#,
    ...
    fn plan_npx_mode_emits_npx_command_without_toolkit_root() {
        ...
        assert_eq!(args[0], "-y");
        assert_eq!(args[1], "unity-open-mcp@0.8.3");
...
"#,
    ...
    fn claude_mcp_add_command_npx_mode_omits_node_prefix() {
        ...
        assert!(cmd.contains("-- npx -y unity-open-mcp@0.8.3"));
        assert!(!cmd.contains("-- node "));
    }
