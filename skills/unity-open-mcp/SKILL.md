...
Common groups: `gate-and-verify`, `asset-intelligence`, `typed-editor`, `diagnostics`, `gate-intelligence`, `build-settings`, `navigation`, `input-system`, `probuilder`, `particle-system`, `animation`, `splines`, `lighting`, `audio`, `agent-senses`. Compiled-state availability (`available: true/false/null`) reflects whether the Unity domain package compiled in (built-in modules like Lighting and Audio are always compiled — `available: true`); the authoritative source is `unity_open_mcp_capabilities` → `toolGroups[].available`. State resets to `core`-only on MCP-server restart.
