...
- **Predict (proactive):** `unity_open_mcp_resource_pressure` samples the live Unity process's fd usage and reports headroom against Mono's ~1024 fd ceiling (NOT the OS soft limit, which is misleading for a GUI-launched Unity on macOS). Returns `state` (`ok`/`warn`/`critical`/`unknown`), a `trend` (`stable`/`rising`/`leaking` — a monotonic climb across successive samples is the leak signature), and the session-scoped sample ring. Sample after heavy automation to catch fd growth before the Editor hangs.

**Workflow.** After heavy automation, call `resource_pressure`. When `state` is `warn`/`critical` or `trend.state` is `leaking`, surface the risk to the operator (recommend saving scene work + planning a restart) — do NOT call `restart_editor` yet (the Editor is still healthy). Once `read_compile_errors` reports an actual `editor_fd_exhaustion` issue (the Editor is hung), call `restart_editor` with `confirm: true`, then tell the operator to relaunch via the Hub. Both tools act on the OS process — neither depends on the bridge being reachable.
