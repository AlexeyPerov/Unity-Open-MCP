...
- **Kill (reactive):** `unity_open_mcp_restart_editor` with `confirm: true` terminates the hung Unity (SIGTERM → SIGKILL on macOS/Linux; `taskkill /T /F` on Windows). It refuses when the `editor_fd_exhaustion` signature is absent — never restart on a fixable compile failure. **Relaunch is NOT automatic** (the Hub owns the interactive-Editor launch recipe); the response tells the operator to relaunch via the Hub, then poll `bridge_status` until `running`.
