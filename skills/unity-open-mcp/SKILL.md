...
- `**reassign_missing_texture**` (unsafe) — assigns a texture to the material's null texture slot(s). Dry-run advertises candidate textures; apply requires `target_texture` (asset path or GUID). Pass `target_property` (from the issue's `evidence.property`) to narrow to a single slot — **without it the texture is written into EVERY null slot**, corrupting optional slots like `_BumpMap`/`_OcclusionMap`/`_EmissionMap` on a Standard material. Producer: `materials`. Never auto-applied.
