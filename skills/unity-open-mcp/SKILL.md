...
- `unity_open_mcp_ping` should report `connected: true` for live bridge flows. A successful answer is always a fresh probe (`source: "probe"` + `asOf`); when it also carries `lockCheck` (the project's instance lock is `gone` while a listener answered), the responder may be a dying Editor or a foreign process on the port — confirm with `bridge_status` before issuing live mutations.
...
1. **Discover first** — call `unity_open_mcp_capabilities` before assuming tool names, schemas, route policy, or fixes. It honors the same `profile` / `page_size` / `cursor` contract as the heavy reads and defaults to `profile: "compact"` (no per-tool `inputSchema`, no descriptions); narrow with `kind: "tools"` + `page_size`, then re-call `profile: "full"` for the one schema you actually need. Never call it at `full` unpaged — that is ~500 KB.
...
4. **One test run at a time** — never start a second `unity_senses_run_tests` before the first resolves (results cross-contaminate). `run_id` names the run a call STARTS; it is **not** a poll handle, and re-calling with it is refused while that run is in flight. There is nothing to poll: the call waits out `timeout_ms` and returns the terminal result (raise it for a long suite). It also cannot be a `batch_execute` step (`batch_step_requires_server_poll`).
...
7. **Start compact, expand on demand** — the heavy tools (`read_asset` / `search_assets` / `scene_get_data` / `find_references` / `validate_edit` / `scan_paths` / `component_get`) default to `profile: "compact"` + a `pagination` block when `page_size` is set. Drill down (`property_path` on `component_get`; `component` / `path` / `id` on asset reads) or escalate to `balanced` / `full` only for the slice you need; set `page_size` to bound any profile. See `capabilities.costHints` for per-tool profile cost bands + recommended page sizes.

## Fast start sequence

1. `unity_open_mcp_capabilities` — discover surface (call first on a fresh project).
2. Activate the tools you need. **Prefer the intent path** over hand-picking group ids:
   - `unity_open_mcp_manage_tools(action="activate_for", intent="…")` — state what you are about to do (e.g. `"bake a NavMesh for the dungeon"`, `"verify the prefab references and run a scan"`, `"edit the player prefab and its materials"`); the server activates the recommended groups in one call and emits `tools/list_changed` when the visible set changes. Mutating / verify-ish intents additionally surface `gate-intelligence` so the pre/post-mutation intelligence tools (impact_preview, gate_budget_estimate, mutation_explain) come online before mutations run. Idempotent — re-calls are no-ops.
   - Use `action="suggest"` with the same `intent` / `tags` to preview the recommendation **without** changing state (read-only).
   - `tags: ["navigation", "audio", …]` narrow or replace the free-text match; accepted forms are canonical tag names, catalog keywords, or group ids. Unknown tags are reported in `unmatchedTags` and otherwise ignored.
   - Fall back to `action="list_groups"` to browse every group with its tool roster, then `action="activate"` / `deactivate"` one id at a time. Sessions start with two default groups enabled: `core` and `gate-and-verify`. State is per-session and ephemeral.
...
- **Predict (proactive):** `unity_open_mcp_resource_pressure` samples the live Unity process's fd usage and reports headroom against the Mono fd ceiling (default ~1024, NOT the OS soft limit, which is misleading for a GUI-launched Unity on macOS). Returns `state` (`ok` / `warn` at ≥80% / `critical` at ≥90% / `over_ceiling` / `unknown`), a `trend` (`stable`/`rising`/`leaking` — a monotonic climb across successive samples is the leak signature), `ceiling` + `ceilingSource` (`"default"` vs `"config"`), and the session-scoped sample ring. Sample after heavy automation to catch fd growth before the Editor hangs. **Fd-heavy projects:** an asset-heavy Editor legitimately sits OVER the ceiling proxy on a fresh healthy process (`lsof`/`HandleCount` count ALL OS fds — mmap'd assets, file watchers, `FileStream` handles — while only Mono-IOSelector-registered descriptors trip the hang), so `over_ceiling` is INFORMATIONAL, not an alarm: when `state` is `over_ceiling` and the trend is stable/no-history the response carries a `pressureNote` (no `warning`) — watch the **trend**, not the absolute. Override the ceiling for a runtime whose internal limit differs from Mono's 1024 (e.g. Unity 6 / CoreCLR) via `.unity-open-mcp/settings.json` (`resourcePressure.fdCeiling`).

**Workflow.** After heavy automation, call `resource_pressure`. When `state` is `warn`/`critical` or `trend.state` is `leaking`/`rising`, surface the risk to the operator (recommend saving scene work + planning a restart) — do NOT call `restart_editor` yet (the Editor is still healthy). A stable/no-history `over_ceiling` state on an fd-heavy project is NOT a warning (it carries a `pressureNote`) — only treat it as a concern if the trend becomes `rising`/`leaking`. Once `read_compile_errors` reports an actual `editor_fd_exhaustion` issue (the Editor is hung), call `restart_editor` with `confirm: true`, then tell the operator to relaunch via the Hub. Both tools act on the OS process — neither depends on the bridge being reachable.
...
**Scenes** (`paths_hint` = scene asset path) — `scene_create` (`setup: empty|default`, `mode: single|additive`; syncs the in-memory scene name to the asset filename stem after saving, so name-only lookups by the stem resolve without a separate open) / `scene_open` (Single is `restart_then_settle` — dirty guard preflights; `ignore_scene_dirty: true` skips) / `scene_save` (active scene when `name` omitted; idempotent on clean) / `scene_unload` (refuses last opened scene) / `scene_set_active`. Read-only: `scene_list_opened` / `scene_get_data` (`profile: compact|balanced|full`, `depth`, `max_nodes` — reflects unsaved editor state, unlike `read_asset` on the `.unity`; `page_size`/`cursor` page the node stream) / `scene_get_dirty_summary`. SceneView tools: `scene_focus` frames a GameObject (`axis: top|bottom|front|back|left|right`), `sceneview_get_camera` reads the current SceneView pose (position/rotation/pivot/projection/size/fov), and `sceneview_set_camera` sets pose-level camera state (position required, optional rotation/orthographic/size) and returns `windowMoved: true`.

**Scene identity is path-first.** `scene_set_active` / `scene_unload` / `scene_get_data` resolve an opened scene by `path` (asset path, e.g. `Assets/Scenes/Foo.unity`) with `name` as fallback; when both are supplied, `path` wins. `scene_save`'s `path` is dual-purpose: when it matches an opened scene's asset path it selects that scene and saves to its own path (identity); otherwise it is a save-as destination for the active/`name`-selected scene. Prefer `path` for robustness — Unity's in-memory scene names can drift from the on-disk asset, especially after additive create.
